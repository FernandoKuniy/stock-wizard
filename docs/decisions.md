...
- 2026-08-22  **The keep-warm workflow pings on a loop inside one run (seven pings, five minutes apart) instead of once per scheduled run, retries for ~100s instead of 30s, and fails only when every ping failed** (the `*/10` cron was never honoured: across 200 runs GitHub's real cadence was a median of 34 minutes, up to 112, so 97% of pings hit an already-cold instance and the 47-60s run times were cold boots, meaning the job woke Render rather than keeping it warm. The job itself is not throttled, so the loop is where real cadence is available, and it is free on a public repo. The old 2 retries at 15s covered 30s, less than a cold boot, so a waking instance could fail the job that exists to keep it awake; that is exactly what the two 502/503 failures on 2026-08-20 were. Failing only on a total blackout turns the alert back into signal.) Rejected `pg_cron` in Supabase despite its better cadence: a paused project stops running its own cron, so it cannot be the thing that prevents the pause, and `pg_net` is fire-and-forget, which would have traded a watchdog that emails on failure for one that fails silently.
- 2026-08-19  The invite code travels through email confirmation in Supabase `user_metadata` and is redeemed automatically at `/auth/confirm`, instead of being asked for again on `/redeem` after a manual sign-in (signup had no session to redeem with and the form was gone by the time one existed, so a new user typed the code, signed in by hand, then typed the code again; metadata is a carrier only, and the API still verifies it, so a user editing their own metadata gains nothing).
- 2026-08-19  `/auth/confirm` accepts both `token_hash` and `code` links, rather than requiring the Supabase email template to be customised (the `token_hash` form verifies standalone and so survives signing up on one device and confirming on another, but the default template keeps working same-browser, which means the flow is not broken by a dashboard setting nobody remembered to change).
- 2026-08-19  `proxy.ts` lets `/auth` through regardless of session state, rather than adding it to the public paths (a public path redirects a *signed-in* user to the dashboard, which would skip the automatic redeem when a confirmation link is clicked twice).
